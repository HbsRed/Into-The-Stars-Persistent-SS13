...
/proc/create_account(var/new_owner_name = "Default user", var/starting_funds = 0, var/obj/machinery/computer/account_database/source_db)
	var/datum/money_account/Mo = get_account_record(new_owner_name)
	if(Mo)
		message_admins("duplicate account loaded owner: [new_owner_name] account_number: [Mo.account_number]")
		return Mo
